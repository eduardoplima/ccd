            ...
            <li key={stage.key} className="flex items-center gap-3">
              ...
              {state === "done" || state === "running" ? (
                <span className="text-xs text-muted-foreground">
                  {state === "done" ? "concluído" : "executando"} · {counter}{" "}
                  {stage.key === "decisoes" ? "decisões" : "itens"}
                </span>
              ) : null}
            </li>
          );
...
export default function EtlPage() {
  ...
  const abort = useAbortExtracao();
  const del = useDeleteExtracao();
  const active = useExtracao(activeId);
  const [filters, setFilters] = useState<ExtracaoListFilters>({});
              ...
              {listLoading && items.length === 0 ? (
                ...
                items.map((item) => (
                  ...
                  >
                    ...
                    <TableCell>
                      <Link href={`/cgad/etl/${item.id}`} className="block w-full">
                      ...
                      <div className="flex items-center justify-end gap-1">
                        {item.status === "queued" || item.status === "running" ? (
                          <Button
                            ...
                            aria-label="Cancelar extração"
                            disabled={abort.isPending}
                            onClick={() => {
                              if (!window.confirm("Cancelar esta extração?")) return;
                          ...
                          <Button
                            ...
                            aria-label="Remover extração"
                            disabled={del.isPending}
                            onClick={() => {
                              if (
                                !window.confirm(
                                  "Remover esta extração do histórico? Os eventos serão apagados; obrigações e recomendações já geradas permanecem.",
                                )
                              )
                                return;
                              del.mutate(item.id, {
...
function FilterBar({
  ...
  onChange: (f: ExtracaoListFilters) => void;
}) {
  function update<K extends keyof ExtracaoListFilters>(
    key: K,
    value: ExtracaoListFilters[K] | undefined,
  ) {
    const next = { ...filters };
    ...
    <div className="flex flex-wrap items-end gap-3 rounded-md border bg-muted/30 p-3">
      ...
      <div className="flex flex-col gap-1">
        ...
        <Input
          ...
          onChange={(e) => update("start_date_to", e.target.value || undefined)}
        />
      </div>
      {hasActiveFilters ? (
