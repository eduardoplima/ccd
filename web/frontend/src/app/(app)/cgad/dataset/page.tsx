...
type ListaTab = "pending" | "done" | "entidades" | "vazios";
type Tab = ListaTab | "divergencias";

const FILTROS: Record<ListaTab, { status?: string; comEntidades?: boolean }> = {
...
export default function DatasetPage() {
  ...
  const filtro = tab === "divergencias" ? FILTROS.pending : FILTROS[tab];
  ...
  if (filtro.comEntidades !== undefined) {
    contexto.set("com_entidades", String(filtro.comEntidades));
  }
  ...
  return (
    <main className="mx-auto flex w-full max-w-screen-2xl flex-col gap-4 p-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          As abas de probabilidade são uma triagem: um documento cai em{" "}
              ...
              {progresso.data.concordancia.some((c) => c.documentos_comuns > 0) && (
                <p className="pt-2 text-sm text-muted-foreground">
                  ...
                  {progresso.data.concordancia
                    ...
                    .map(
                      (c) =>
                        `${c.a}×${c.b} ${(c.f1! * 100).toFixed(1)}% (${c.documentos_comuns} docs)`,
                    )
                    .join(" · ")}
                </p>
              )}
