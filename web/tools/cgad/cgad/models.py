...
class ExtracaoORM(Base):
    ...
    eventos = relationship(
        "ExtracaoEventoORM",
        back_populates="extracao",
        cascade="all, delete-orphan",
        order_by="ExtracaoEventoORM.IdExtracaoEvento",
    )


class ExtracaoEventoORM(Base):
    """Live activity log for an extraction run.

    One row per granular event the orchestrator emits as it progresses —
    a per-decision feed that powers the live UI on ``/etl/[id]``. Events are
    persisted (not just pushed via pub/sub) so reopening an old extraction
    replays the entire timeline.

    ``Tipo`` examples:
      - ``stage_started`` / ``stage_done`` — boundary events for each pipeline stage.
      - ``decision_done`` — one NER decision finished; ``Payload`` carries the
        identity triple plus per-decision counters (multas/obrigações/etc.).
      - ``obrigacao_extracted`` / ``recomendacao_extracted`` — a stage-2 final
        row was written; ``Payload`` carries the new ``IdObrigacao`` /
        ``IdRecomendacao`` and the originating decision triple.
      - ``error`` — a failure that the orchestrator caught and recorded
        without aborting the whole run; ``Payload.message`` has the message.

    ``Payload`` is JSON to keep schemas flexible without adding columns per
    new event type.
    """

    __tablename__ = "ExtracaoEvento"

    IdExtracaoEvento = Column(Integer, primary_key=True, autoincrement=True)
    IdExtracao = Column(
        Integer,
        ForeignKey("Extracao.IdExtracao"),
        nullable=False,
        index=True,
    )
