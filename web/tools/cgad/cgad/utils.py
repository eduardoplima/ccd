...
def get_connection(db: str = 'processo') -> Engine:
    ...
    connection_string = "mssql+pyodbc:///?odbc_connect=" + urllib.parse.quote_plus(odbc)
...
def _prepare_extraction_context(
    ...
    extractor_citacao: BaseChatModel,
):
...
def extract_obrigacao(
    ...
    row, responsible, citacao = _prepare_extraction_context(
        row, extractor_responsible, extractor_citacao
    )
...
def save_ner_decision(
    ...
    # Multas
    for idx, multa in enumerate(ner_decision.multas or []):
    ...
    # Ressarcimentos
    for idx, ressarcimento in enumerate(ner_decision.ressarcimentos or []):
    ...
    # Obrigações
    for idx, obrigacao in enumerate(ner_decision.obrigacoes or []):
    ...
    # Recomendações
    for idx, recomendacao in enumerate(ner_decision.recomendacoes or []):
...
""".strip()
    ...
    # ``extractor`` must already be wrapped via ``with_structured_output(CitationChoice)``;
    # callers pass a pre-wrapped sequence built in the worker factory.
    choice: CitationChoice = extractor.invoke(prompt)
    idx = choice.index
    chosen = filtered[idx]
