...
# Row positions of the leaked few-shot examples in the 866-document
# evaluation outputs under dataset/results/output/. Verified empirically
# across all LLM result JSONs: dataset id N sits at row position N - 1.
FEWSHOT_RESULT_POSITIONS = [i - 1 for i in FEWSHOT_DATASET_IDS]
...
label_8 = NERDecisao(
    ...
    obrigacoes=[
        NERObrigacao(
            descricao_obrigacao="obrigação de remeter a este Tribunal de Contas, quando das nomeações, o demonstrativo que certifique a existência de prévia dotação orçamentária suficiente para atender às projeções de despesa com pessoal e aos acréscimos dela decorrentes quando da nomeação dos candidatos aprovados",
        ),
        NERObrigacao(
            descricao_obrigacao="obrigação de remeter a este Tribunal de Contas os processos de admissão, com o fito de proceder à análise de sua legalidade, com fins de registro",
        ),
        NERObrigacao(
            descricao_obrigacao="Comprovar, em até quatro quadrimestres – contados a partir desta Decisão –, sob pena de ser considerado nulo de pleno direito o concurso público, que o Poder Executivo adotou as medidas para eliminar o percentual excedente do limite prudencial de despesas com pessoal – com a adoção das medidas descritas na Constituição Federal e na LRF –, considerando o disposto nos arts. 20, 22, 23, e 66, todos da Lei de Responsabilidade Fiscal, inclusive com a rescisão de todos os contratos temporários",
        ),
        NERObrigacao(
...
def get_formatted_messages_from_examples(
    examples: List[Tuple[str, str]],
) -> List[Dict[str, BaseMessage]]:
    ...
    for text_input, tool_call in examples:
        ...
        formatted_messages.extend(
            convert_tool_example_to_messages(
                {"input": text_input, "tool_calls": [tool_call]}
            )
        )
