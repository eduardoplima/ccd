...
Lê os PDFs dos apensados do processo no share, pergunta ao DeepSeek (Foundry
...
class ParcelaExtraida(BaseModel):
    ...
    valor: float = Field(description="Valor da parcela em reais")
...
def trecho_confere(trecho: str, texto: str, minimo: int = 25) -> bool:
    """O começo do trecho citado tem que aparecer no texto normalizado."""
    t = _normalizar(trecho)
    if len(t) < minimo:
        return False
    return t[:60] in _normalizar(texto)
...
def _gravar(
    ...
    *,
    ...
    if resposta and resposta.encontrou:
        linhas = (
            [(p.numero, p.mes, p.ano, p.valor) for p in resposta.parcelas if p.valor > 0]
...
def extrair_resposta(id_cadastro: int) -> str:
    ...
    with Session(get_processo_engine()) as sp:
        ...
        textos = [
            (ap, *_texto_dos_pdfs(processo_lookup.informacoes(sp, ap["numero"], ap["ano"])))
            for ap in apensos
        ]
    ...
    for ap, texto_ap, arquivos in textos:
        ...
        evento = processo_lookup.evento_apensamento(infos_principal, ap["data_registro"])
    ...
    return "nenhum apensado informa o valor do desconto"
