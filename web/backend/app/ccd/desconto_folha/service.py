...
def _q_clause_pessoa_orgao(col_pessoa: str, col_orgao: str, col_cpf: str) -> str:
    return f"({_like_ci_ai(col_pessoa)} OR {_like_ci_ai(col_orgao)} OR {col_cpf} LIKE :q_like)"
...
def obter_cadastro_manual(session: Session, id_desconto_folha: int) -> CadastroManualDetail | None:
    row = (
        session.execute(
            text(
                """
            SELECT
                df.IdFRAPDescontoFolha, df.Origem, df.CpfCnpj, df.NomePessoa,
                df.IdOrgaoNotificado, df.NomeOrgaoNotificado,
                df.QtdParcelasPlanejadas, df.ValorTotalEsperado, df.DataInclusao
            FROM dbo.FRAPDescontoFolha df
            WHERE df.IdFRAPDescontoFolha = :id AND df.Origem = 'M' AND df.Ativo = 1
            """
            ),
            {"id": id_desconto_folha},
        )
    ...
    parcelas = (
        session.execute(
            text(
                """
            SELECT
                p.IdFRAPDescontoFolhaParcela, p.NumeroParcela,
                p.MesReferencia, p.AnoReferencia, p.ValorEsperado, p.DataVencimento,
                (SELECT COUNT(*) FROM dbo.FRAPMatchDescontoFolha m
                  WHERE m.IdFRAPDescontoFolhaParcela = p.IdFRAPDescontoFolhaParcela) AS QtdMatch
            FROM dbo.FRAPDescontoFolhaParcela p
            WHERE p.IdFRAPDescontoFolha = :id
            ORDER BY p.AnoReferencia, p.MesReferencia, p.NumeroParcela
            """
            ),
            {"id": id_desconto_folha},
        )
    ...
    return CadastroManualDetail(
        id_desconto_folha=int(row["IdFRAPDescontoFolha"]),
        origem=str(row["Origem"]),
        cpfcnpj=row["CpfCnpj"],
        nome_pessoa=row["NomePessoa"],
        id_orgao_notificado=row["IdOrgaoNotificado"],
        nome_orgao_notificado=row["NomeOrgaoNotificado"],
        qtd_parcelas=int(row["QtdParcelasPlanejadas"] or 0),
        valor_total=Decimal(str(row["ValorTotalEsperado"] or 0)),
        data_inclusao=row["DataInclusao"],
        parcelas=[
            ParcelaCadastroItem(
                id_frap_parcela=int(p["IdFRAPDescontoFolhaParcela"]),
                numero_parcela=p["NumeroParcela"],
                mes_referencia=p["MesReferencia"],
                ano_referencia=p["AnoReferencia"],
                valor_esperado=Decimal(str(p["ValorEsperado"] or 0)),
                data_vencimento=p["DataVencimento"],
                tem_match=int(p["QtdMatch"]) > 0,
            )
            for p in parcelas
        ],
    )
...
def atualizar_cadastro_manual(
    session: Session, id_desconto_folha: int, *, dados: dict[str, Any]
) -> str:
    """Edita nome/órgão do cadastro manual. Retorna 'ok', 'not_found', 'not_manual'."""
    row = session.execute(
        text("SELECT Origem FROM dbo.FRAPDescontoFolha WHERE IdFRAPDescontoFolha = :id"),
        {"id": id_desconto_folha},
    ).first()
    ...
    colunas = {
        ...
        "nome_orgao_notificado": "NomeOrgaoNotificado",
    }
    dados = {k: v for k, v in dados.items() if k in colunas}
    if not dados:
        return "ok"
    sets = ", ".join(f"{colunas[c]} = :{c}" for c in dados)
    session.execute(
        text(f"UPDATE dbo.FRAPDescontoFolha SET {sets} WHERE IdFRAPDescontoFolha = :id"),
        {**dados, "id": id_desconto_folha},
    )
    session.commit()
    return "ok"


def _recalcular_agregados_cadastro(session: Session, id_desconto_folha: int) -> None:
    """Mantém QtdParcelasPlanejadas/ValorTotalEsperado coerentes após mutar parcelas."""
    session.execute(
        text(
            """
            UPDATE df
            ...
            WHERE df.IdFRAPDescontoFolha = :id
            """
        ),
        {"id": id_desconto_folha},
    )
...
def _validar_pai_manual(session: Session, id_desconto_folha: int) -> str:
    row = session.execute(
        text("SELECT Origem FROM dbo.FRAPDescontoFolha WHERE IdFRAPDescontoFolha = :id"),
        {"id": id_desconto_folha},
    ).first()
...
def criar_parcela_manual(
    session: Session, id_desconto_folha: int, *, parcela: dict[str, Any]
) -> int | str:
    """Retorna o id da parcela criada ou 'not_found'/'not_manual'."""
    guard = _validar_pai_manual(session, id_desconto_folha)
    if guard != "ok":
        return guard
    id_parcela = int(
        session.execute(
            text(
                """
                INSERT INTO dbo.FRAPDescontoFolhaParcela
                    (IdFRAPDescontoFolha, IdParcela, NumeroParcela, MesReferencia,
                     AnoReferencia, ValorEsperado, DataVencimento)
                OUTPUT inserted.IdFRAPDescontoFolhaParcela
                VALUES (:id_pai, NULL, :numero_parcela, :mes, :ano, :valor, :dt_venc)
                """
            ),
            {
                "id_pai": id_desconto_folha,
                "numero_parcela": int(parcela["numero_parcela"]),
                "mes": int(parcela["mes_referencia"]),
                "ano": int(parcela["ano_referencia"]),
                "valor": Decimal(str(parcela["valor_esperado"])),
                "dt_venc": parcela.get("data_vencimento"),
            },
        ).scalar_one()
    )
    _recalcular_agregados_cadastro(session, id_desconto_folha)
    session.commit()
    return id_parcela


def _obter_parcela_manual(session: Session, id_desconto_folha: int, id_parcela: int) -> str:
    """Guard: parcela existe, pertence ao pai, pai é manual, parcela é manual."""
    guard = _validar_pai_manual(session, id_desconto_folha)
    if guard != "ok":
        return guard
    row = session.execute(
        text(
            """
            SELECT IdParcela FROM dbo.FRAPDescontoFolhaParcela
            WHERE IdFRAPDescontoFolhaParcela = :id_parcela
              AND IdFRAPDescontoFolha = :id_pai
            """
        ),
        {"id_parcela": id_parcela, "id_pai": id_desconto_folha},
    ).first()
    if row is None:
        return "not_found"
...
def atualizar_parcela_manual(
    session: Session, id_desconto_folha: int, id_parcela: int, *, dados: dict[str, Any]
) -> str:
    """Retorna 'ok', 'not_found', 'not_manual'."""
    guard = _obter_parcela_manual(session, id_desconto_folha, id_parcela)
    if guard != "ok":
        return guard
    colunas = {
    ...
    if "valor_esperado" in dados:
        dados["valor_esperado"] = Decimal(str(dados["valor_esperado"]))
    sets = ", ".join(f"{colunas[c]} = :{c}" for c in dados)
    session.execute(
        text(
            f"UPDATE dbo.FRAPDescontoFolhaParcela SET {sets} "
            "WHERE IdFRAPDescontoFolhaParcela = :id_parcela"
        ),
        {**dados, "id_parcela": id_parcela},
    )
    _recalcular_agregados_cadastro(session, id_desconto_folha)
    session.commit()
    return "ok"
...
def deletar_parcela_manual(session: Session, id_desconto_folha: int, id_parcela: int) -> str:
    """Retorna 'ok', 'not_found', 'not_manual', 'has_match'."""
    guard = _obter_parcela_manual(session, id_desconto_folha, id_parcela)
    if guard != "ok":
        return guard
    n_match = int(
        session.execute(
            text(
                "SELECT COUNT(*) FROM dbo.FRAPMatchDescontoFolha "
                "WHERE IdFRAPDescontoFolhaParcela = :id_parcela"
            ),
    ...
    session.execute(
        text(
            "DELETE FROM dbo.FRAPDescontoFolhaParcela "
            "WHERE IdFRAPDescontoFolhaParcela = :id_parcela"
        ),
        {"id_parcela": id_parcela},
    )
