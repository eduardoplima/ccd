-- Débitos de execução cujo responsável (Processo_TransitoJulgado.Responsavel)
-- casa com o padrão de nome (LIKE, em minúsculas). Portado de
-- ccd/sql/processos_transito_nome.sql.
SELECT
    ...
    (SELECT Descricao
        ...
        WHERE etd.CodigoTipoDebito = e.CodigoTipoDebito) as tipo_debito,
    ...
    (SELECT DescricaoStatusDivida
        ...
        WHERE esd.CodigoStatusDivida = e.CodigoStatusDivida) as situacao_divida,
    (SELECT Descricao
        FROM processo.dbo.Exe_StatusProtesto esp
        WHERE esp.IdExe_StatusProtesto = e.StatusProtesto) as status_protesto,
    (SELECT DescricaoStatusProcessoPGE
        FROM processo.dbo.PGE_StatusProcesso psp
        WHERE psp.IdStatusProcessoPGE = e.Status_PGE) as status_pge,
    gp.Documento as cpf,
    gp.Nome as nome
FROM processo.dbo.Exe_Debito e
INNER JOIN processo.dbo.Exe_DebitoPessoa edp ON e.IdDebito = edp.IDDebito
INNER JOIN processo.dbo.GenPessoa gp ON gp.IdPessoa = edp.IDPessoa
WHERE IdProcessoOrigem IN (
    SELECT p.IdProcesso
    FROM processo.dbo.Processos p
    INNER JOIN processo.dbo.Processo_TransitoJulgado ptj
        ON p.numero_processo = ptj.numero_processo
        AND p.ano_processo = ptj.ano_processo
    WHERE lower(ptj.Responsavel) LIKE :nome_pattern
)
